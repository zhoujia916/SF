...
(function (factory) {
	...
	var Tree = function (element, options) {
		...
		this.$element.on('click.fu.tree', '.icon-folder', $.proxy( function(ev) { this.openFolder(ev.currentTarget); }, this));
        this.$element.on('click.fu.tree', '.tree-branch-header .tree-branch-name', $.proxy( function(ev) { this.selectItem(ev.currentTarget); }, this));
		//ACE
	...
	Tree.prototype = {
		...
		populate: function ($el) {
			...
			this.options.dataSource( treeData ? treeData : {} , function (items) {
				...
				$.each( items.data, function(index, value) {
					...
					if(value.type === 'folder' || ('additionalParameters' in value)) {
		...
		openFolder: function (el) {
			var $el = $(el.parentNode.parentNode); // tree-branch-name
			var $branch;
